...
20. **Akcje WP na linkach ([BŁĄD]):** każdy edytowalny `<a>` ma `cms-block-field-type="link"` + `cms-block-field-control="link"` (albo `cms-editable-type="link"` / `cms-post-link` / `cms-site-link`). Wykryj i zgłoś jako [BŁĄD]: `cms-block-field-type="attr"` z `cms-block-field-attribute="href"` (zła akcja, klient traci picker podstron). Tekst przycisku musi być osobnym polem content (span albo `cms-block-field-2`); link bez edytowalnej etykiety tam, gdzie klient będzie ją zmieniał = [OSTRZEŻENIE].
...
22. **Layout sekcji:** sekcja główna trzyma wzorzec `px-5` na `<section>` + `max-w-site mx-auto` na kontenerze + `py-*` na min. 2 breakpointach. Odstępstwo bez powodu (fullbleed slider, sekcja dekoracyjna) = [OSTRZEŻENIE].
