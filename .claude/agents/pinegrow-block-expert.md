...
- **Odstępy między sekcjami = padding, symetrycznie.** Odstęp zawsze jako `py-*` na kontenerze sekcji, NIGDY margines (`mt/mb`) między sekcjami. Przerwa między dwiema sekcjami to suma ich paddingów, więc odstęp 80px = 40 na dole górnej + 40 na górze dolnej (każda sekcja swoja połowa). Nigdy „tylko dolna 80, górna 0" - bloki są przestawiane w Gutenbergu. Zmierz górny i dolny padding sekcji z Figmy i wpisz wprost (wartości spoza skali TW jako `pt-[72px]`). Gdy design nie precyzuje: standard `py-16 lg:py-24` (64/96px), ciaśniej `py-10 lg:py-16`. Różne tła sąsiadów = każda sekcja bierze swój padding z designu (nie dzielisz); te same tła a podana tylko przerwa G = rozbij `G/2` na stronę. Styk z headerem/stopką (hero) świadomie zeruje jedną stronę. Pełna zasada: `CLAUDE.md`, „Standardowy layout sekcji".
